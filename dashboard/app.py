...
def api_sessions(limit: int = 20):
    ...
    cur = conn.cursor()
    cur.execute("""
        SELECT id, date, bodyweight, notes FROM sessions
        ORDER BY id DESC LIMIT ?
    """, (limit,))
    rows = cur.fetchall()
    conn.close()
    return [{"id": r[0], "date": r[1], "bodyweight": r[2], "notes": r[3]} for r in rows]
...
def api_session_detail(session_id: int):
    ...
    cur.execute("SELECT id, name FROM exercises WHERE session_id = ? ORDER BY id", (session_id,))
    exercises = []
    for ex_id, ex_name in cur.fetchall():
        cur.execute("SELECT load, reps FROM sets WHERE exercise_id = ? ORDER BY id", (ex_id,))
        sets = [{"load": r[0], "reps": r[1]} for r in cur.fetchall()]
        e1rm = max((r["load"] * (1 + r["reps"] / 30.0) for r in sets), default=0)
        exercises.append({"name": ex_name, "sets": sets, "top_e1rm": round(e1rm, 1)})
    conn.close()
    return {"id": s[0], "date": s[1], "bodyweight": s[2], "notes": s[3], "exercises": exercises}

@app.get("/api/analytics/tonnage")
def api_tonnage(limit: int = 30):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("""
        SELECT s.date, SUM(st.load * st.reps) as tonnage
        FROM sessions s
        JOIN exercises ex ON ex.session_id = s.id
        JOIN sets st ON st.exercise_id = ex.id
        GROUP BY s.id ORDER BY s.id DESC LIMIT ?
    """, (limit,))
    rows = cur.fetchall()
    conn.close()
    return [{"date": r[0], "tonnage": round(r[1] or 0)} for r in reversed(rows)]

@app.get("/api/analytics/e1rm/{movement}")
def api_e1rm_history(movement: str, limit: int = 20):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("""
        SELECT s.date, MAX(st.load * (1 + st.reps / 30.0)) as e1rm
        FROM sessions s
        JOIN exercises ex ON ex.session_id = s.id
        JOIN sets st ON st.exercise_id = ex.id
        WHERE LOWER(ex.name) LIKE LOWER(?)
        GROUP BY s.id ORDER BY s.id DESC LIMIT ?
    """, (f"%{movement}%", limit))
    rows = cur.fetchall()
    conn.close()
    return [{"date": r[0], "e1rm": round(r[1] or 0, 1)} for r in reversed(rows)]

@app.post("/api/sessions", status_code=201)
def api_log_session(payload: SessionIn):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute(
